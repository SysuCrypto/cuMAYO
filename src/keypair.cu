...
__global__ void gpu_keypair(uint8_t *d_sk,uint8_t *d_pk,uint8_t *d_eP3,uint8_t *d_P,uint8_t *d_P2,size_t pitch,size_t rand_index){
    ...
    __syncwarp();
    // generate a per-instance counter as the seed
    // this deterministic construction is only used for test-vector/benchmark reproducibility
    // in real applications, the seed must be generated by a cryptographically secure RNG
