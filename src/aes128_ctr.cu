// aes128_tables.cu

#include "aes128_ctr.cuh"
#include <stdint.h>

__device__ __constant__ u32 d_T0_base[256];
__device__ __constant__ u8  d_SBOX_base[256];

__device__ __constant__ u32 d_RCON32[10] = {
    0x01000000, 0x02000000, 0x04000000, 0x08000000,
    0x10000000, 0x20000000, 0x40000000, 0x80000000,
    0x1b000000, 0x36000000
};

static const u8  h_SBOX[256] = {
    
};

static const u32 h_T0[256] = {0xc66363a5U, 0xf87c7c84U, 0xee777799U, 0xf67b7b8dU, 0xfff2f20dU, 0xd66b6bbdU, 0xde6f6fb1U, 0x91c5c554U,
	0x60303050U, 0x02010103U, 0xce6767a9U, 0x562b2b7dU,	0xe7fefe19U, 0xb5d7d762U, 0x4dababe6U, 0xec76769aU,
	0x8fcaca45U, 0x1f82829dU, 0x89c9c940U, 0xfa7d7d87U,	0xeffafa15U, 0xb25959ebU, 0x8e4747c9U, 0xfbf0f00bU,
	0x41adadecU, 0xb3d4d467U, 0x5fa2a2fdU, 0x45afafeaU,	0x239c9cbfU, 0x53a4a4f7U, 0xe4727296U, 0x9bc0c05bU,
	0x75b7b7c2U, 0xe1fdfd1cU, 0x3d9393aeU, 0x4c26266aU,	0x6c36365aU, 0x7e3f3f41U, 0xf5f7f702U, 0x83cccc4fU,
	0x6834345cU, 0x51a5a5f4U, 0xd1e5e534U, 0xf9f1f108U,	0xe2717193U, 0xabd8d873U, 0x62313153U, 0x2a15153fU,
	0x0804040cU, 0x95c7c752U, 0x46232365U, 0x9dc3c35eU,	0x30181828U, 0x379696a1U, 0x0a05050fU, 0x2f9a9ab5U,
	0x0e070709U, 0x24121236U, 0x1b80809bU, 0xdfe2e23dU,	0xcdebeb26U, 0x4e272769U, 0x7fb2b2cdU, 0xea75759fU,
	0x1209091bU, 0x1d83839eU, 0x582c2c74U, 0x341a1a2eU,	0x361b1b2dU, 0xdc6e6eb2U, 0xb45a5aeeU, 0x5ba0a0fbU,
	0xa45252f6U, 0x763b3b4dU, 0xb7d6d661U, 0x7db3b3ceU,	0x5229297bU, 0xdde3e33eU, 0x5e2f2f71U, 0x13848497U,
	0xa65353f5U, 0xb9d1d168U, 0x00000000U, 0xc1eded2cU,	0x40202060U, 0xe3fcfc1fU, 0x79b1b1c8U, 0xb65b5bedU,
	0xd46a6abeU, 0x8dcbcb46U, 0x67bebed9U, 0x7239394bU,	0x944a4adeU, 0x984c4cd4U, 0xb05858e8U, 0x85cfcf4aU,
	0xbbd0d06bU, 0xc5efef2aU, 0x4faaaae5U, 0xedfbfb16U,	0x864343c5U, 0x9a4d4dd7U, 0x66333355U, 0x11858594U,
	0x8a4545cfU, 0xe9f9f910U, 0x04020206U, 0xfe7f7f81U,	0xa05050f0U, 0x783c3c44U, 0x259f9fbaU, 0x4ba8a8e3U,
	0xa25151f3U, 0x5da3a3feU, 0x804040c0U, 0x058f8f8aU,	0x3f9292adU, 0x219d9dbcU, 0x70383848U, 0xf1f5f504U,
	0x63bcbcdfU, 0x77b6b6c1U, 0xafdada75U, 0x42212163U,	0x20101030U, 0xe5ffff1aU, 0xfdf3f30eU, 0xbfd2d26dU,
	0x81cdcd4cU, 0x180c0c14U, 0x26131335U, 0xc3ecec2fU,	0xbe5f5fe1U, 0x359797a2U, 0x884444ccU, 0x2e171739U,
	0x93c4c457U, 0x55a7a7f2U, 0xfc7e7e82U, 0x7a3d3d47U,	0xc86464acU, 0xba5d5de7U, 0x3219192bU, 0xe6737395U,
	0xc06060a0U, 0x19818198U, 0x9e4f4fd1U, 0xa3dcdc7fU,	0x44222266U, 0x542a2a7eU, 0x3b9090abU, 0x0b888883U,
	0x8c4646caU, 0xc7eeee29U, 0x6bb8b8d3U, 0x2814143cU,	0xa7dede79U, 0xbc5e5ee2U, 0x160b0b1dU, 0xaddbdb76U,
	0xdbe0e03bU, 0x64323256U, 0x743a3a4eU, 0x140a0a1eU,	0x924949dbU, 0x0c06060aU, 0x4824246cU, 0xb85c5ce4U,
	0x9fc2c25dU, 0xbdd3d36eU, 0x43acacefU, 0xc46262a6U,	0x399191a8U, 0x319595a4U, 0xd3e4e437U, 0xf279798bU,
	0xd5e7e732U, 0x8bc8c843U, 0x6e373759U, 0xda6d6db7U,	0x018d8d8cU, 0xb1d5d564U, 0x9c4e4ed2U, 0x49a9a9e0U,
	0xd86c6cb4U, 0xac5656faU, 0xf3f4f407U, 0xcfeaea25U,	0xca6565afU, 0xf47a7a8eU, 0x47aeaee9U, 0x10080818U,
	0x6fbabad5U, 0xf0787888U, 0x4a25256fU, 0x5c2e2e72U,	0x381c1c24U, 0x57a6a6f1U, 0x73b4b4c7U, 0x97c6c651U,
	0xcbe8e823U, 0xa1dddd7cU, 0xe874749cU, 0x3e1f1f21U,	0x964b4bddU, 0x61bdbddcU, 0x0d8b8b86U, 0x0f8a8a85U,
	0xe0707090U, 0x7c3e3e42U, 0x71b5b5c4U, 0xcc6666aaU,	0x904848d8U, 0x06030305U, 0xf7f6f601U, 0x1c0e0e12U,
	0xc26161a3U, 0x6a35355fU, 0xae5757f9U, 0x69b9b9d0U,	0x17868691U, 0x99c1c158U, 0x3a1d1d27U, 0x279e9eb9U,
	0xd9e1e138U, 0xebf8f813U, 0x2b9898b3U, 0x22111133U,	0xd26969bbU, 0xa9d9d970U, 0x078e8e89U, 0x339494a7U,
	0x2d9b9bb6U, 0x3c1e1e22U, 0x15878792U, 0xc9e9e920U,	0x87cece49U, 0xaa5555ffU, 0x50282878U, 0xa5dfdf7aU,
	0x038c8c8fU, 0x59a1a1f8U, 0x09898980U, 0x1a0d0d17U,	0x65bfbfdaU, 0xd7e6e631U, 0x844242c6U, 0xd06868b8U,
	0x824141c3U, 0x299999b0U, 0x5a2d2d77U, 0x1e0f0f11U,	0x7bb0b0cbU, 0xa85454fcU, 0x6dbbbbd6U, 0x2c16163aU,};



void aes128_tables_init() {
    cudaMemcpyToSymbol(d_T0_base,   h_T0,   256 * sizeof(u32));
    cudaMemcpyToSymbol(d_SBOX_base, h_SBOX, 256 * sizeof(u8));
}

__device__ __forceinline__ u32 rotr8 (u32 x) { return __funnelshift_r(x, x,  8); }
__device__ __forceinline__ u32 rotr16(u32 x) { return __funnelshift_r(x, x, 16); }
__device__ __forceinline__ u32 rotr24(u32 x) { return __funnelshift_r(x, x, 24); }

__device__ __forceinline__ u32 subword_const(u32 x){
    return (u32)d_SBOX_base[(x >> 24) & 0xff] << 24 |
           (u32)d_SBOX_base[(x >> 16) & 0xff] << 16 |
           (u32)d_SBOX_base[(x >>  8) & 0xff] <<  8 |
           (u32)d_SBOX_base[(x      ) & 0xff];
}
__device__ __forceinline__ u32 rotword(u32 x){ return (x << 8) | (x >> 24); }

__device__ __forceinline__ void aes128_expand_key_lane0(const u8 key[16], u32 rk[44]) {
    u32 k0 = (u32(key[0])<<24)|(u32(key[1])<<16)|(u32(key[2])<<8)|key[3];
    u32 k1 = (u32(key[4])<<24)|(u32(key[5])<<16)|(u32(key[6])<<8)|key[7];
    u32 k2 = (u32(key[8])<<24)|(u32(key[9])<<16)|(u32(key[10])<<8)|key[11];
    u32 k3 = (u32(key[12])<<24)|(u32(key[13])<<16)|(u32(key[14])<<8)|key[15];

    rk[0]=k0; rk[1]=k1; rk[2]=k2; rk[3]=k3;

    #pragma unroll
    for (int i = 4; i < 44; i += 4) {
        u32 temp = rk[i - 1];
        temp = rotword(temp);
        temp = subword_const(temp);
        temp ^= d_RCON32[(i/4) - 1];
        rk[i + 0] = rk[i - 4] ^ temp;
        rk[i + 1] = rk[i + 0] ^ rk[i - 3];
        rk[i + 2] = rk[i + 1] ^ rk[i - 2];
        rk[i + 3] = rk[i + 2] ^ rk[i - 1];
    }
}

__device__ __forceinline__ void store16_be(u8* out, u32 s0, u32 s1, u32 s2, u32 s3) {
    out[0]=(u8)(s0>>24); out[1]=(u8)(s0>>16); out[2]=(u8)(s0>>8); out[3]=(u8)s0;
    out[4]=(u8)(s1>>24); out[5]=(u8)(s1>>16); out[6]=(u8)(s1>>8); out[7]=(u8)s1;
    out[8]=(u8)(s2>>24); out[9]=(u8)(s2>>16); out[10]=(u8)(s2>>8); out[11]=(u8)s2;
    out[12]=(u8)(s3>>24); out[13]=(u8)(s3>>16); out[14]=(u8)(s3>>8); out[15]=(u8)s3;
}

__device__ __forceinline__ void store_tail_be(u8* out, u32 s0, u32 s1, u32 s2, u32 s3, u32 n) {
    #pragma unroll
    for (u32 i = 0; i < 16; ++i) {
        if (i >= n) break;
        u32 w = (i < 4) ? s0 : (i < 8) ? s1 : (i < 12) ? s2 : s3;
        u32 shift = 24u - 8u * (i & 3u);
        out[i] = (u8)(w >> shift);
    }
}

__device__ void aes128_ctr(const u8* key16, u8* out, size_t out_len)
{
    const int lane = threadIdx.x & 31;

    __shared__ u32 rkS[44];
    __shared__ u32 T0S[256];
    __shared__ u8  SboxS[256];

    for (int t = lane; t < 256; t += 32) {
        T0S[t]   = d_T0_base[t];
        SboxS[t] = d_SBOX_base[t];
    }

    if (lane == 0) aes128_expand_key_lane0(key16, rkS);
    __syncwarp();

    const u32 total_blocks = (u32)(out_len >> 4);
    const u32 tail_bytes   = (u32)(out_len & 0xFu);

    for (u32 blk = (u32)lane; blk < total_blocks; blk += 32u) {
        const u64 counter = (u64)blk;

        u32 s0 = 0, s1 = 0;
        u32 s2 = (u32)(counter >> 32);
        u32 s3 = (u32)counter;

        s0 ^= rkS[0]; s1 ^= rkS[1]; s2 ^= rkS[2]; s3 ^= rkS[3];

        #pragma unroll
        for (int round = 0; round < 9; ++round) {
            const u32 rk_off = (u32)(round + 1) * 4;

            u32 t0 = T0S[(s0 >> 24)       ] ^
                     rotr8 (T0S[(s1 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s2 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s3      ) & 0xFF]) ^
                     rkS[rk_off + 0];

            u32 t1 = T0S[(s1 >> 24)       ] ^
                     rotr8 (T0S[(s2 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s3 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s0      ) & 0xFF]) ^
                     rkS[rk_off + 1];

            u32 t2 = T0S[(s2 >> 24)       ] ^
                     rotr8 (T0S[(s3 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s0 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s1      ) & 0xFF]) ^
                     rkS[rk_off + 2];

            u32 t3 = T0S[(s3 >> 24)       ] ^
                     rotr8 (T0S[(s0 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s1 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s2      ) & 0xFF]) ^
                     rkS[rk_off + 3];

            s0 = t0; s1 = t1; s2 = t2; s3 = t3;
        }

        auto SB = [&](u32 x)->u32 { return (u32)SboxS[x]; };

        u32 o0 = (SB((s0 >> 24)       ) << 24) ^
                 (SB((s1 >> 16) & 0xFF) << 16) ^
                 (SB((s2 >>  8) & 0xFF) <<  8) ^
                 (SB((s3      ) & 0xFF)      ) ^
                 rkS[40];

        u32 o1 = (SB((s1 >> 24)       ) << 24) ^
                 (SB((s2 >> 16) & 0xFF) << 16) ^
                 (SB((s3 >>  8) & 0xFF) <<  8) ^
                 (SB((s0      ) & 0xFF)      ) ^
                 rkS[41];

        u32 o2 = (SB((s2 >> 24)       ) << 24) ^
                 (SB((s3 >> 16) & 0xFF) << 16) ^
                 (SB((s0 >>  8) & 0xFF) <<  8) ^
                 (SB((s1      ) & 0xFF)      ) ^
                 rkS[42];

        u32 o3 = (SB((s3 >> 24)       ) << 24) ^
                 (SB((s0 >> 16) & 0xFF) << 16) ^
                 (SB((s1 >>  8) & 0xFF) <<  8) ^
                 (SB((s2      ) & 0xFF)      ) ^
                 rkS[43];

        u8* pout = out + ((size_t)blk << 4);
        store16_be(pout, o0, o1, o2, o3);
    }

    if (tail_bytes && lane == 0) {
        const u32 blk = total_blocks;
        const u64 counter = (u64)blk;

        u32 s0 = 0, s1 = 0;
        u32 s2 = (u32)(counter >> 32);
        u32 s3 = (u32)counter;

        s0 ^= rkS[0]; s1 ^= rkS[1]; s2 ^= rkS[2]; s3 ^= rkS[3];

        #pragma unroll
        for (int round = 0; round < 9; ++round) {
            const u32 rk_off = (u32)(round + 1) * 4;

            u32 t0 = T0S[(s0 >> 24)       ] ^
                     rotr8 (T0S[(s1 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s2 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s3      ) & 0xFF]) ^
                     rkS[rk_off + 0];

            u32 t1 = T0S[(s1 >> 24)       ] ^
                     rotr8 (T0S[(s2 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s3 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s0      ) & 0xFF]) ^
                     rkS[rk_off + 1];

            u32 t2 = T0S[(s2 >> 24)       ] ^
                     rotr8 (T0S[(s3 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s0 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s1      ) & 0xFF]) ^
                     rkS[rk_off + 2];

            u32 t3 = T0S[(s3 >> 24)       ] ^
                     rotr8 (T0S[(s0 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s1 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s2      ) & 0xFF]) ^
                     rkS[rk_off + 3];

            s0 = t0; s1 = t1; s2 = t2; s3 = t3;
        }

        auto SB0 = [&](u32 x)->u32 { return (u32)SboxS[x]; };

        u32 o0 = (SB0((s0 >> 24)       ) << 24) ^
                 (SB0((s1 >> 16) & 0xFF) << 16) ^
                 (SB0((s2 >>  8) & 0xFF) <<  8) ^
                 (SB0((s3      ) & 0xFF)      ) ^
                 rkS[40];

        u32 o1 = (SB0((s1 >> 24)       ) << 24) ^
                 (SB0((s2 >> 16) & 0xFF) << 16) ^
                 (SB0((s3 >>  8) & 0xFF) <<  8) ^
                 (SB0((s0      ) & 0xFF)      ) ^
                 rkS[41];

        u32 o2 = (SB0((s2 >> 24)       ) << 24) ^
                 (SB0((s3 >> 16) & 0xFF) << 16) ^
                 (SB0((s0 >>  8) & 0xFF) <<  8) ^
                 (SB0((s1      ) & 0xFF)      ) ^
                 rkS[42];

        u32 o3 = (SB0((s3 >> 24)       ) << 24) ^
                 (SB0((s0 >> 16) & 0xFF) << 16) ^
                 (SB0((s1 >>  8) & 0xFF) <<  8) ^
                 (SB0((s2      ) & 0xFF)      ) ^
                 rkS[43];

        u8* pout = out + ((size_t)blk << 4);
        store_tail_be(pout, o0, o1, o2, o3, tail_bytes);
    }
}


__device__ void aes128_ctr_warp(const u8* key16, u8* out, size_t out_len)
{
    const int tid  = threadIdx.x;
    const int lane = tid & 31;

    __shared__ u32 rkS[44];
    __shared__ u32 T0S[256];
    __shared__ u8  SboxS[256];


    for (int t = lane; t < 256; t += 32) {
            T0S[t]   = d_T0_base[t];
            SboxS[t] = d_SBOX_base[t];
        }

    if (tid == 0) {
        aes128_expand_key_lane0(key16, rkS);
    }

    __syncthreads();

    const u32 total_blocks = (u32)(out_len >> 4);
    const u32 tail_bytes   = (u32)(out_len & 0xFu);

    for (u32 blk = (u32)tid; blk < total_blocks; blk += (u32)blockDim.x) {
        const u64 counter = (u64)blk;

        u32 s0 = 0, s1 = 0;
        u32 s2 = (u32)(counter >> 32);
        u32 s3 = (u32)counter;

        s0 ^= rkS[0]; s1 ^= rkS[1]; s2 ^= rkS[2]; s3 ^= rkS[3];

        #pragma unroll
        for (int round = 0; round < 9; ++round) {
            const u32 rk_off = (u32)(round + 1) * 4;

            u32 t0 = T0S[(s0 >> 24)       ] ^
                     rotr8 (T0S[(s1 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s2 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s3      ) & 0xFF]) ^
                     rkS[rk_off + 0];

            u32 t1 = T0S[(s1 >> 24)       ] ^
                     rotr8 (T0S[(s2 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s3 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s0      ) & 0xFF]) ^
                     rkS[rk_off + 1];

            u32 t2 = T0S[(s2 >> 24)       ] ^
                     rotr8 (T0S[(s3 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s0 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s1      ) & 0xFF]) ^
                     rkS[rk_off + 2];

            u32 t3 = T0S[(s3 >> 24)       ] ^
                     rotr8 (T0S[(s0 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s1 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s2      ) & 0xFF]) ^
                     rkS[rk_off + 3];

            s0 = t0; s1 = t1; s2 = t2; s3 = t3;
        }

        auto SB = [&](u32 x)->u32 { return (u32)SboxS[x]; };

        u32 o0 = (SB((s0 >> 24)       ) << 24) ^
                 (SB((s1 >> 16) & 0xFF) << 16) ^
                 (SB((s2 >>  8) & 0xFF) <<  8) ^
                 (SB((s3      ) & 0xFF)      ) ^
                 rkS[40];

        u32 o1 = (SB((s1 >> 24)       ) << 24) ^
                 (SB((s2 >> 16) & 0xFF) << 16) ^
                 (SB((s3 >>  8) & 0xFF) <<  8) ^
                 (SB((s0      ) & 0xFF)      ) ^
                 rkS[41];

        u32 o2 = (SB((s2 >> 24)       ) << 24) ^
                 (SB((s3 >> 16) & 0xFF) << 16) ^
                 (SB((s0 >>  8) & 0xFF) <<  8) ^
                 (SB((s1      ) & 0xFF)      ) ^
                 rkS[42];

        u32 o3 = (SB((s3 >> 24)       ) << 24) ^
                 (SB((s0 >> 16) & 0xFF) << 16) ^
                 (SB((s1 >>  8) & 0xFF) <<  8) ^
                 (SB((s2      ) & 0xFF)      ) ^
                 rkS[43];

        u8* pout = out + ((size_t)blk << 4);
        store16_be(pout, o0, o1, o2, o3);
    }

    if (tail_bytes && tid == 0) {
        const u32 blk = total_blocks;
        const u64 counter = (u64)blk;

        u32 s0 = 0, s1 = 0;
        u32 s2 = (u32)(counter >> 32);
        u32 s3 = (u32)counter;

        s0 ^= rkS[0]; s1 ^= rkS[1]; s2 ^= rkS[2]; s3 ^= rkS[3];

        #pragma unroll
        for (int round = 0; round < 9; ++round) {
            const u32 rk_off = (u32)(round + 1) * 4;

            u32 t0 = T0S[(s0 >> 24)       ] ^
                     rotr8 (T0S[(s1 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s2 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s3      ) & 0xFF]) ^
                     rkS[rk_off + 0];

            u32 t1 = T0S[(s1 >> 24)       ] ^
                     rotr8 (T0S[(s2 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s3 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s0      ) & 0xFF]) ^
                     rkS[rk_off + 1];

            u32 t2 = T0S[(s2 >> 24)       ] ^
                     rotr8 (T0S[(s3 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s0 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s1      ) & 0xFF]) ^
                     rkS[rk_off + 2];

            u32 t3 = T0S[(s3 >> 24)       ] ^
                     rotr8 (T0S[(s0 >> 16) & 0xFF]) ^
                     rotr16(T0S[(s1 >>  8) & 0xFF]) ^
                     rotr24(T0S[(s2      ) & 0xFF]) ^
                     rkS[rk_off + 3];

            s0 = t0; s1 = t1; s2 = t2; s3 = t3;
        }

        auto SB0 = [&](u32 x)->u32 { return (u32)SboxS[x]; };

        u32 o0 = (SB0((s0 >> 24)       ) << 24) ^
                 (SB0((s1 >> 16) & 0xFF) << 16) ^
                 (SB0((s2 >>  8) & 0xFF) <<  8) ^
                 (SB0((s3      ) & 0xFF)      ) ^
                 rkS[40];

        u32 o1 = (SB0((s1 >> 24)       ) << 24) ^
                 (SB0((s2 >> 16) & 0xFF) << 16) ^
                 (SB0((s3 >>  8) & 0xFF) <<  8) ^
                 (SB0((s0      ) & 0xFF)      ) ^
                 rkS[41];

        u32 o2 = (SB0((s2 >> 24)       ) << 24) ^
                 (SB0((s3 >> 16) & 0xFF) << 16) ^
                 (SB0((s0 >>  8) & 0xFF) <<  8) ^
                 (SB0((s1      ) & 0xFF)      ) ^
                 rkS[42];

        u32 o3 = (SB0((s3 >> 24)       ) << 24) ^
                 (SB0((s0 >> 16) & 0xFF) << 16) ^
                 (SB0((s1 >>  8) & 0xFF) <<  8) ^
                 (SB0((s2      ) & 0xFF)      ) ^
                 rkS[43];

        u8* pout = out + ((size_t)blk << 4);
        store_tail_be(pout, o0, o1, o2, o3, tail_bytes);
    }
}

